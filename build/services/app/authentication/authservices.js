...
const authservices = {
  ...
  registerUserService: function (user) {
    ...
    return fetch(authurl + '/register', config).then(response => response.json().then(data => ({
  ...
  loginUserService: function (creds) {
    ...
    return fetch(authurl + '/oauth/token', config).then(response => response.json().then(user => ({
      user,
      response
    }))).catch(function (err) {
