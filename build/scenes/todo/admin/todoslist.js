...
export default class TodosList extends React.Component {
  ...
  getItems() {
    ...
    if (this.props.todos) {
      return this.props.todos.filter(item => item.get('completed') && this.props.filterclosed || !item.get('completed') && this.props.filteropen);
  ...
  renderItems() {
    ...
    return items.map((todo, index) => {
      ...
      return React.createElement(TodosListItem, { ind: index, todo: todo, actions: this.props.actions });
  ...
  render() {
    ...
    return React.createElement(
      'div',
      { className: 'todoslist' },
      this.props.todos == undefined ? React.createElement('div', { className: 'mdl-spinner mdl-spinner--single-color mdl-js-spinner is-active', style: { width: '55px', height: '55px' } }) : React.createElement(
        'span',
        null,
        this.renderItems()
      )
